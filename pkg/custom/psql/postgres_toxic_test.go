...
func TestFailAfter(t *testing.T) {
	...
	given.
		a_connection_to_postgres().and().
		a_psql_toxic(options{FailAfter: 2, SearchText: ".*SELECT 1.*"})
	when.
		a_query_succeeds("SELECT 1").and().
		a_query_succeeds("SELECT 1").and()
	then.
...
func TestFailConnectionAfter(t *testing.T) {
	...
	given.
		a_connection_to_postgres().and().
		a_psql_toxic(options{FailAfter: 2, SearchText: ".*SELECT 1.*", FailureType: FailureTypeConnectionFailure})
	when.
		a_query_succeeds("SELECT 1").and().
		a_query_succeeds("SELECT 1").and()
	then.
...
func TestRecoverAfter(t *testing.T) {
	...
	given.
		a_connection_to_postgres().and().
		a_psql_toxic(options{FailAfter: 2, RecoverAfter: 3, SearchText: ".*SELECT 1.*"})
	when.
		a_query_succeeds("SELECT 1").and().
		a_query_succeeds("SELECT 1").and().
		a_query_fails("SELECT 1", ErrMessageFailure)
